...
- 分解証明の確立（2026-09-22、`docs/solver-decomp-musou-007-2026-09-22.json`）:
  記録ライン上の各後手番で全応手を列挙しdf-pnで個別証明する深さ優先手法。
  musou-007で全27応手の詰みを証明（20は9手以内・6は11〜21手・1は23手、
  最長分岐で初手から24手以内）。zukou-013は21応手中15まで証明、残り6は
  escalation予定。本手法を全問横展開する（1問あたり数分CPUの見込み）。
